...
private:
  ...
  std::map<std::string, std::vector<double>> upPosture = {{"joint_6", {M_PI / 4 + moveMag}}};
  std::map<std::string, std::vector<double>> downPosture = {{"joint_6", {M_PI / 4 - moveMag}}};

  // StoreState
  void storeState();
  StoreState store_state_ = START;
  std::map<std::string, std::vector<double>> storePoseStart = {
      {"joint_1", {0.5 * M_PI}}, {"joint_2", {0}},   {"joint_3", {0}}, {"joint_4", {0}},
      {"joint_5", {0}},          {"joint_6", {2.0}}, {"joint_7", {0}}};
  std::map<std::string, std::vector<double>> storePoseFold = {{"joint_4", {2.3}}, {"joint_5", {0.4}}};
  std::map<std::string, std::vector<double>> storePoseBend = {{"joint_1", {0.0}}, {"joint_2", {0.5 * M_PI}}};
};
